...
Ext.define('ONELINERS.controller.Oneliners', {
    ...
    init: function() {
        ...
        var viewPort = Ext.create('Ext.container.Viewport', {
            ...
            items: [{
                region: 'north',
                html: '<div><h1 class="x-panel-header" style="float: left">OWASP 1-Liner SECURISH</h1><h1 id="loggedInAs" class="x-panel-header" style="float: right; padding-right: 1em">Logged in as ' + Ext.util.Cookies.get("nickNameSecurish") + '</h1></div>',
                autoHeight: true,
                border: false,
                margins: '0 0 5 0'
            }, {
...
//        viewPort.add(profilePanel);
    ...
    insertOneLiner: function(oneLinerStr) {
        var oneLiner = ONELINERS.model.Oneliner.create({  // Ext.create?
            "nickName": Ext.util.Cookies.get("nickNameSecurish"),
...
var pollNumberOfOneliners = function() {
    Ext.Ajax.request({
        url: 'ws/securish/oneliners/size',
...
var pollUserName = function() {
    Ext.core.DomHelper.overwrite(Ext.getDom("loggedInAs"), "Logged in as " + Ext.util.Cookies.get("nickNameSecurish"));
}
